...
const CommentForm = ({ stream,index , classes}) => {
  ...
  const createCommentHandler = (e) => {
    e.preventDefault();
    if(userInfo){
    dispatch(createComment(stream._id, index , post));
    setBody("");
  }
  };
  return (
    <>
      <Grid item sm={12} style={{ textAlign: "center" }}>
        <form onSubmit={createCommentHandler}>
          ...
          ></TextField>
          {stream.errorCreateCommente && (
          <MuiAlert severity="error">{stream.errorCreateCommente}</MuiAlert>
        )}
        <br />
          {stream.loadingCreateComment ? (
